...
resource "aws_lambda_function" "products_lambda" {
  function_name = "${var.global.environment}-products-lambda"
  role          = aws_iam_role.lambda_role.arn
  package_type  = "Image"
  image_uri     = var.lambda_config.products_ecr_image_uri
  ...
  tags = {
    Name        = "${var.global.environment}-products-lambda"
    Environment = var.global.environment
    Service     = "products"
  }
}
...
resource "aws_iam_role" "lambda_role" {
  name = "${var.global.environment}-products-lambda-role"
...
resource "aws_cloudwatch_log_group" "lambda_log_group" {
  name              = "/aws/lambda/${aws_lambda_function.products_lambda.function_name}"
  retention_in_days = 14

  tags = {
    Name        = "${var.global.environment}-products-lambda-logs"
    Environment = var.global.environment
    Service     = "products"
  }
}
